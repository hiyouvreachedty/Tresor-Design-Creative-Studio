...
const ProjectDetailColumn: React.FC<ProjectDetailColumnProps> = ({
  project,
  onInfoClick,
  onEnlargeClick,
}) => {
      ...
      {project ? (
        <div className="flex-1 flex flex-col min-h-0">
            ...
            {project.tickerSymbol ? (
                ...
                {project.images?.[0] && (
                  <div className="group aspect-square bg-gray-800/50 rounded-lg overflow-hidden transition-opacity duration-500">
                    <img
                      ...
                      className="w-full h-full object-cover animate-fade-in transition-transform duration-300 ease-in-out group-hover:scale-105"
                    />
                  </div>
                )}
              ...
              project.images.slice(0, 4).map((image, index) => (
                <div
                  ...
                  className="group aspect-square bg-gray-800/50 rounded-lg overflow-hidden transition-opacity duration-500"
                >
                  <img
                    src={image}
          ...
          <div className="mt-auto flex justify-between items-center p-2 bg-[#0D1117]/20 rounded-lg backdrop-blur-sm border border-white/10">
            ...
            >
              <span className="border border-gray-500 w-5 h-5 flex items-center justify-center">
                +
              </span>
              <span>ENLARGE</span>
            </button>
